...
class AppBlockerService : AccessibilityService() {
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        // ANTI-UNINSTALL PROTECTION: Block Settings when trying to uninstall this app
        if (AntiUninstallHelper.isProtectionEnabled(this)) {
            if (isSettingsApp(pkg)) {
                detectAndBlockUninstallAttempt(event)
            }
        }
    ...
    /**
     * Check if package is Settings app
     */
    ...
    /**
     * Detect and block uninstall attempts in Settings
     */
    private fun detectAndBlockUninstallAttempt(event: AccessibilityEvent) {
        try {
            val source = event.source ?: return
            val text = source.text?.toString()?.lowercase() ?: ""
            val className = event.className?.toString()?.lowercase() ?: ""
            
            // Detect uninstall-related UI elements
            val uninstallKeywords = listOf(
                "uninstall", "hapus", "remove", "delete",
                "force stop", "hentikan paksa", "disable", "nonaktifkan"
            )
            
            val isUninstallUI = uninstallKeywords.any { keyword ->
                text.contains(keyword) || className.contains(keyword)
            }
            
            if (isUninstallUI) {
                android.util.Log.w("AppBlocker", "⚠️ UNINSTALL ATTEMPT DETECTED!")
                
                // Aggressive block - spam HOME multiple times
                for (i in 0..5) {
                    mainHandler.postDelayed({
                        performGlobalAction(GLOBAL_ACTION_HOME)
                    }, i * 100L)
                ...
                // Show warning toast
                mainHandler.postDelayed({
                    Toast.makeText(this, "⚠️ Proteksi sistem aktif", Toast.LENGTH_LONG).show()
                }, 200)
                
                // Launch our app to cover Settings
                mainHandler.postDelayed({
            ...
            android.util.Log.e("AppBlocker", "detectAndBlockUninstallAttempt: ${e.message}")
        }
    }
